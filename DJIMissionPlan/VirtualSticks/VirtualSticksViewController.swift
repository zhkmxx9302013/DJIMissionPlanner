...
class VirtualSticksViewController: UIViewController, MKMapViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        if let aircraft = DJISDKManager.product() as? DJIAircraft {
            ...
            if let fc = aircraft.flightController {
                ...
                self.missionButton.setTitle("开始任务", for: .normal)
            }
        }
    }
    ...
    @IBAction func startVSMission(_ sender: UIButton) {
        ...
        self.missionButton.setTitle("正在跟踪航点", for: .normal)
...
//        }
//
//          let alert = UIAlertView.init(title: "", message: msg, delegate: nil, cancelButtonTitle: "Ok", otherButtonTitles:"取消")
//                alert.show()
    ...
    @objc func tapPress(_ gestureRecognizer:UITapGestureRecognizer){
      ...
      let touchMapCoordinate:CLLocationCoordinate2D = self.mapView.convert(touchPoint, toCoordinateFrom: self.mapView)
        GPSController.setWayPoint(latitude: touchMapCoordinate.latitude, longitude: touchMapCoordinate.longitude)
    }
    ...
    func startVSStarNow(grid: [[Double]], speed: Float = 4) {
        ...
        self.queue.asyncAfter(deadline: .now() + 1.0) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                ...
                self.missionButton.setTitleColor(UIColor.white, for: .normal)
                self.missionButton.setTitle("执行航点跟踪任务", for: .normal)
            }
        }
    }
